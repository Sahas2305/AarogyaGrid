...
def update_bill(billing_id):
    """
    Admin → can update any bill (payment_method, amount, payment_date).
    Patient → can only pay their OWN pending bill (sets payment_method only).
    ...
    try:
        ...
        # Fetch the existing bill first
        existing = supabase.table('billing') \
            .select('billing_id, patient_id, payment_method') \
        ...
        # Patients can only pay their own unpaid bills
        if user['role'] == 'patient':
            if bill['patient_id'] != user['patient_id']:
                return jsonify({'error': 'Access denied. You can only pay your own bills.'}), 403
