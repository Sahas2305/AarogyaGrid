...
export const PatientDashboard = () => {
  ...
  // Compute real banner values
  const lastVisit    = pastAppts.find(a => a.status === 'Completed')?.date || null;
  const nextAppt     = upcomingAppts[0]?.date || null;
  const outstandingAmt = bills
    .filter(b => !b.payment_method && !b.status?.toLowerCase().includes('paid'))
    ...
    <div className="space-y-6 select-none">
      ...
      <div className="bg-gradient-to-r from-[#112255] to-[#0a1628] rounded-2xl border border-white/8 p-6 flex flex-col sm:flex-row sm:items-center justify-between shadow-xl">
        ...
        <div className="mt-4 sm:mt-0 flex flex-wrap gap-2.5">
          <Badge variant="cyan">Last Visit: {lastVisit || 'No visits yet'}</Badge>
          <Badge variant="purple">Next Appt: {nextAppt || 'None booked'}</Badge>
          {outstandingAmt > 0
            ? <Badge variant="danger">Outstanding: ₹{outstandingAmt.toLocaleString('en-IN')}</Badge>
            : <Badge variant="success">No Outstanding Bills</Badge>
          }
        </div>
      </div>
              ...
              {loading ? <SkeletonLoader rows={2} /> : upcomingAppts.length === 0 ? (
                ...
                <Card key={app.appointment_id} className="p-4 bg-surface-card border border-white/5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-text-secondary mt-0.5">{app.department_name} Department</p>
