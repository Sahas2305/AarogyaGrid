...
import { Calendar, User, Clock, ChevronRight, ChevronLeft, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export const BookAppointment = () => {
  ...
  const handleNext = () => {
    if (step === 1 && !selectedDoctor) {
      toast.error('Please select a physician to consult.');
      return;
    }
  ...
  const handleConfirmBooking = () => {
    toast.loading('Registering appointment ledger and generating database record...');
    setTimeout(() => {
      toast.dismiss();
      ...
      <div className="flex items-center justify-between px-2">
        {[1, 2, 3, 4].map((i) => (
          <React.Fragment key={i}>
            ...
            {i < 4 && (
              <div className={`flex-1 h-[2px] mx-4 rounded transition-all ${step > i ? 'bg-brand-cyan' : 'bg-white/5'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      {step === 1 && (
        <Card className="p-5 space-y-4 bg-surface-card border border-white/5">
          <div className="border-b border-white/5 pb-2">
            ...
            <p className="text-xs text-text-secondary mt-1">Select an active physician from Dayananda Hospital roster</p>
          </div>
            ...
            {mockDoctors.map((doc) => (
              ...
              >
                <div>
                  ...
                  <span className="text-[9px] font-mono text-brand-cyan block mt-1">{doc.department} dept • {doc.floor}F</span>
                </div>
                <Badge variant={doc.status === 'Available' ? 'success' : 'warning'} className="text-[8px] py-0">
                  {doc.status}
                </Badge>
              </div>
            ))}
      ...
      {step === 2 && (
        <Card className="p-5 space-y-4 bg-surface-card border border-white/5">
          <div className="border-b border-white/5 pb-2">
            ...
            <p className="text-xs text-text-secondary mt-1">Configure appointment timing calendars</p>
          </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-2.5">
                {slots.map((s, idx) => (
                  <button
                    ...
                    className={`
                      py-2 rounded-lg text-xs font-mono font-semibold transition-all border
                      ${!s.available
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex bg-[#0a1628]/40 border border-white/5 p-1 rounded-xl w-60">
                <button
                  type="button"
                  onClick={() => setApptFormat('In-Person')}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    apptFormat === 'In-Person' ? 'bg-brand-cyan text-[#0a1628]' : 'text-text-secondary'
                  }`}
                >
                  In-Person
                </button>
                <button
                  type="button"
                  onClick={() => setApptFormat('Virtual')}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    apptFormat === 'Virtual' ? 'bg-brand-cyan text-[#0a1628]' : 'text-text-secondary'
                  }`}
                >
                  Virtual Tele
                </button>
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                placeholder="List major physical discomforts, pain areas, or follow-up purposes..."
                rows="5"
        ...
        <Card className="p-5 space-y-4 bg-surface-card border border-white/5">
          ...
          <div className="p-4 bg-[#112255]/40 border border-white/5 rounded-2xl space-y-4 text-xs">
            <div className="flex items-center justify-between pb-3.5 border-b border-white/5">
              <span className="font-bold text-white block">Consulting Practitioner:</span>
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-white/5">
              <span className="text-text-secondary block">Format:</span>
            ...
            <div>
              ...
              <p className="text-white mt-1 leading-relaxed italic bg-black/25 p-3 rounded-lg border border-white/5">
                "{complaintText}"
              </p>
            </div>
          </div>
          ...
          <div className="p-3 bg-white/5 border border-white/5 rounded-xl flex items-start space-x-2.5">
            ...
            <span className="text-[9px] text-text-secondary leading-relaxed font-semibold">
              Completing booking assigns a temporary scheduling index and synchronizes notification panels on clinician terminals.
            </span>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/5">
            ...
            <Button onClick={handleConfirmBooking} className="py-2.5 text-xs font-bold bg-gradient-to-r from-brand-cyan to-brand-blue">
              Confirm Appointment
            </Button>
          </div>
        </Card>
