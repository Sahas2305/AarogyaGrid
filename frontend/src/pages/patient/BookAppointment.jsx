 ...
 *   When navigated from AISymptomChecker, reads ai_recommended_specialty from
 *   sessionStorage and highlights / pre-sorts matching doctors with an AI banner.
 * Used on: App.jsx (guarded route /patient/book)
 */
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { toast } from 'react-hot-toast';
import { Calendar, ChevronRight, ChevronLeft, CheckCircle2, Brain, Sparkles, X } from 'lucide-react';
...
export const BookAppointment = () => {
  ...
  // ── AI recommendation from sessionStorage ──────────────────────────────────
  const [aiSpecialty,  setAiSpecialty]  = useState('');
  const [aiCondition,  setAiCondition]  = useState('');
  const [showAiBanner, setShowAiBanner] = useState(false);

  useEffect(() => {
    const specialty = sessionStorage.getItem('ai_recommended_specialty') || '';
    const condition = sessionStorage.getItem('ai_recommended_condition') || '';
    if (specialty) {
      setAiSpecialty(specialty);
      setAiCondition(condition);
      setShowAiBanner(true);
      // Clear so it doesn't re-appear on next fresh visit
      sessionStorage.removeItem('ai_recommended_specialty');
      sessionStorage.removeItem('ai_recommended_condition');
    }
  }, []);
  ...
  useEffect(() => {
    getDoctors()
      ...
      .catch(() => toast.error('Failed to load doctors list.'))
      .finally(() => setLoadingDoctors(false));
  }, []);

  // ── Sort doctors: AI-recommended specialty first ───────────────────────────
  const sortedDoctors = React.useMemo(() => {
    if (!aiSpecialty) return doctors;
    const matches = doctors.filter(d =>
      d.specialization?.toLowerCase().includes(aiSpecialty.toLowerCase()) ||
      aiSpecialty.toLowerCase().includes(d.specialization?.toLowerCase() || '')
    );
    const rest = doctors.filter(d => !matches.includes(d));
    return [...matches, ...rest];
  }, [doctors, aiSpecialty]);
  ...
  // ── Helper: is this doctor AI-recommended? ─────────────────────────────────
  const isRecommended = (doc) => {
    if (!aiSpecialty) return false;
    return (
      doc.specialization?.toLowerCase().includes(aiSpecialty.toLowerCase()) ||
      aiSpecialty.toLowerCase().includes(doc.specialization?.toLowerCase() || '')
    );
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 select-none text-left">
      ...
      {showAiBanner && aiSpecialty && (
        <div className="flex items-start space-x-3 p-4 bg-gradient-to-r from-brand-cyan/10 to-brand-blue/10 border border-brand-cyan/25 rounded-xl">
          <div className="p-1.5 bg-brand-cyan/15 rounded-lg flex-shrink-0">
            <Brain className="w-4 h-4 text-brand-cyan animate-pulse" />
          </div>
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-0.5">
              <Sparkles className="w-3 h-3 text-brand-cyan" />
              <span className="text-[10px] uppercase font-extrabold text-brand-cyan tracking-widest">AI Recommendation</span>
            </div>
            <p className="text-xs text-white font-semibold">
              Based on your symptoms{aiCondition ? ` (${aiCondition})` : ''}, our AI recommends a{' '}
              <span className="text-brand-cyan">{aiSpecialty}</span>.
            </p>
            <p className="text-[10px] text-text-secondary mt-0.5">Recommended doctors are highlighted below.</p>
          </div>
          <button onClick={() => setShowAiBanner(false)} className="text-text-secondary/40 hover:text-white transition-colors flex-shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
        ...
        <Card className="p-5 space-y-4 bg-surface-card border border-white/5">
          <div className="border-b border-white/5 pb-2">
            ...
            <p className="text-xs text-text-secondary mt-1">
              {aiSpecialty
                ? <span>AI suggests a <span className="text-brand-cyan font-bold">{aiSpecialty}</span> — highlighted below</span>
                : 'Select an active physician from the hospital roster'}
            </p>
          </div>
              ...
              {sortedDoctors.map((doc) => {
                ...
                return (
                  <div key={doc.doctor_id}
                    ...
                    className={`p-3.5 rounded-xl border cursor-pointer transition-all flex items-start justify-between relative
                      ${selectedDoctor?.doctor_id === doc.doctor_id
                        ? 'border-brand-cyan bg-brand-cyan/8 shadow'
                        : recommended
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-1.5 mb-0.5">
                        ...
                        {recommended && (
                          <span className="text-[8px] bg-brand-cyan/20 text-brand-cyan border border-brand-cyan/30 px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wide flex items-center space-x-0.5">
                            <Brain className="w-2 h-2" /><span>AI Pick</span>
                          </span>
                        )}
                      </div>
                      <span className="text-[10px] text-text-secondary mt-0.5 block">{doc.specialization}</span>
                      <span className="text-[9px] font-mono text-brand-cyan block mt-1">
                        {typeof doc.department === 'object' ? doc.department?.department_name : doc.department} dept
                      </span>
                    </div>
                    <Badge variant="success" className="text-[8px] py-0 flex-shrink-0 ml-2">Available</Badge>
                  </div>
                );
              })}
            </div>
              ...
              <div className="flex bg-[#0a1628]/40 border border-white/5 p-1 rounded-xl w-60">
                {['In-Person', 'Virtual'].map((fmt) => (
                  <button key={fmt} type="button" onClick={() => setApptFormat(fmt)}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${apptFormat === fmt ? 'bg-brand-cyan text-[#0a1628]' : 'text-text-secondary'}`}>
      ...
      {step === 4 && (
        <Card className="p-5 space-y-4 bg-surface-card border border-white/5">
          <div className="border-b border-white/5 pb-2">
            ...
            <p className="text-xs text-text-secondary mt-1">Audit details before committing</p>
          </div>
          ...
          <div className="p-4 bg-[#112255]/40 border border-white/5 rounded-2xl space-y-4 text-xs">
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-white/5">
              ...
              <span className="text-white font-semibold">{selectedDoctor?.specialization}</span>
            </div>
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-white/5">
              <span className="text-text-secondary">Hospital Facility:</span>
              <span className="text-white font-semibold text-right">{selectedHospital?.name} ({selectedHospital?.city})</span>
            </div>
            <div className="flex items-center justify-between pb-3.5 border-b border-white/5">
            ...
            {aiSpecialty && isRecommended(selectedDoctor) && (
              <div className="flex items-center space-x-2 pt-1">
                <Brain className="w-3.5 h-3.5 text-brand-cyan flex-shrink-0" />
                <span className="text-[10px] text-brand-cyan font-semibold">AI-recommended {aiSpecialty} selected ✓</span>
              </div>
            )}
          </div>
