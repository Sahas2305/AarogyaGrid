...
import { Users, Search, ClipboardList, ExternalLink } from 'lucide-react';
...
export const MyPatients = () => {
  ...
  const filteredPatients = patients.filter(pat =>
    ...
    pat.patient_id.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredPatients.map(pat => {
            ...
            return (
              <Card key={pat.patient_id} className="p-5 bg-surface-card border border-white/5 hover:border-white/10 transition-all duration-200 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between border-b border-white/5 pb-2.5 mb-3">
                    <span className="font-mono text-[10px] text-brand-cyan font-bold tracking-widest">{pat.patient_id}</span>
                    <span className="text-[10px] text-text-secondary font-medium">{pat.registered_date}</span>
                  </div>
                  ...
                  <div className="text-[10px] text-text-secondary/80 mt-3.5 space-y-1 font-mono">
                    <p>Ph: {pat.phone}</p>
                    <p className="truncate">Ins: {pat.insurance_details?.split(' - ')[0]}</p>
                  </div>
                </div>
