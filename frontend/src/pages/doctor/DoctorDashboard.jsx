...
import {
  Calendar, Users, ClipboardList, Brain, Clock, AlertTriangle,
  Save, FileText, FlaskConical, X, User, Phone, MapPin, ShieldCheck
} from 'lucide-react';
...
import { getAppointments, getPatients, createMedicalRecord, getAIDiagnosis, getMedicalRecords, getLabReports } from '../../api/api';
...
export const DoctorDashboard = () => {
  ...
  // Patient Chart Drawer states
  const [chartOpen, setChartOpen] = useState(false);
  const [chartPatient, setChartPatient] = useState(null);
  const [chartAppts, setChartAppts] = useState([]);
  ...
  const handleOpenChart = async (patient) => {
    ...
    try {
      const [allAppts, allRecords, allLabs] = await Promise.all([
        getAppointments(),
        getMedicalRecords(),
        getLabReports(),
      ]);
      // Filter to this patient only (backend scopes to doctor's patients so all data is already relevant)
      setChartAppts(allAppts.filter(a => a.patient_id === patient.patient_id).slice(0, 10));
      setChartRecords(allRecords.filter(r => r.patient_id === patient.patient_id).slice(0, 10));
      setChartLabs(allLabs.filter(l => l.medical_record?.patient_id === patient.patient_id).slice(0, 8));
    } catch (err) {
    ...
    <div className="space-y-6 select-none">
      ...
      {chartPatient && (
        <Drawer
          ...
          title={`Patient Chart: ${chartPatient.name}`}
          size="lg"
        >
          {chartLoading ? (
            ...
            <div className="space-y-6 text-left">
              ...
              {/* Patient Info Card */}
              <div className="p-4 bg-[#112255]/40 border border-white/8 rounded-xl grid grid-cols-2 gap-3 text-xs">
                <div className="flex items-start space-x-2">
                  <User className="w-3.5 h-3.5 text-brand-cyan mt-0.5 flex-shrink-0" />
                  <div>
                    <span className="text-text-secondary block text-[9px] uppercase font-bold tracking-wider">Patient</span>
                    <span className="text-white font-bold">{chartPatient.name}</span>
                  </div>
                </div>
                <div className="flex items-start space-x-2">
                  <span className="text-brand-cyan mt-0.5 text-sm flex-shrink-0">🎂</span>
                  <div>
                    <span className="text-text-secondary block text-[9px] uppercase font-bold tracking-wider">DOB / Age</span>
                    <span className="text-white font-bold">
                      {chartPatient.dob} ({new Date().getFullYear() - new Date(chartPatient.dob).getFullYear()} yrs)
                    </span>
                  </div>
                </div>
                <div className="flex items-start space-x-2">
                  <Phone className="w-3.5 h-3.5 text-brand-cyan mt-0.5 flex-shrink-0" />
                  <div>
                    <span className="text-text-secondary block text-[9px] uppercase font-bold tracking-wider">Phone</span>
                    <span className="text-white font-mono">{chartPatient.phone}</span>
                  </div>
                </div>
                <div className="flex items-start space-x-2">
                  <ShieldCheck className="w-3.5 h-3.5 text-brand-cyan mt-0.5 flex-shrink-0" />
                  <div>
                    <span className="text-text-secondary block text-[9px] uppercase font-bold tracking-wider">Insurance</span>
                    <span className="text-white font-mono">{chartPatient.insurance_details || '—'}</span>
                  </div>
                </div>
                <div className="col-span-2 flex items-start space-x-2">
                  <MapPin className="w-3.5 h-3.5 text-brand-cyan mt-0.5 flex-shrink-0" />
                  <div>
                    <span className="text-text-secondary block text-[9px] uppercase font-bold tracking-wider">Address</span>
                    <span className="text-white">{chartPatient.address}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2 pb-1 border-b border-white/5">
                  ...
                  <span className="text-[10px] uppercase font-extrabold tracking-widest text-brand-cyan">
                    Appointments ({chartAppts.length})
                  </span>
                </div>
                {chartAppts.length === 0 ? (
                  <p className="text-xs text-text-secondary/50 py-2">No appointments found.</p>
                ) : (
                  <div className="space-y-2">
                    {chartAppts.map((a) => (
                      <div key={a.appointment_id} className="flex items-center justify-between p-3 bg-[#112255]/30 border border-white/5 rounded-xl text-xs">
                        <div>
                          <span className="text-white font-bold block">{a.date} @ {a.time_slot}</span>
                          <span className="text-text-secondary text-[10px]">{a.reason || 'No reason provided'}</span>
                        </div>
                        <Badge variant={
                          a.status === 'Completed' ? 'success' :
                          a.status === 'Cancelled' ? 'danger' :
                          a.status === 'Confirmed' ? 'cyan' : 'purple'
                        } className="text-[9px] py-0 px-2 uppercase font-bold flex-shrink-0">
                          {a.status}
                        </Badge>
                      </div>
                    ))}
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2 pb-1 border-b border-white/5">
                  ...
                  <span className="text-[10px] uppercase font-extrabold tracking-widest text-brand-cyan">
                    Medical Records ({chartRecords.length})
                  </span>
                </div>
                {chartRecords.length === 0 ? (
                  <p className="text-xs text-text-secondary/50 py-2">No medical records found.</p>
                ) : (
                  <div className="space-y-2">
                    {chartRecords.map((r) => (
                      <div key={r.record_id} className="p-3 bg-[#112255]/30 border border-white/5 rounded-xl text-xs space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-white font-bold">{r.diagnosis}</span>
                          <span className="text-text-secondary font-mono text-[9px]">{r.record_date}</span>
                        </div>
                        <p className="text-text-secondary text-[10px]"><span className="text-brand-cyan font-bold">Rx:</span> {r.prescription}</p>
                        {r.notes && <p className="text-text-secondary/70 text-[10px] italic">{r.notes}</p>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2 pb-1 border-b border-white/5">
                  ...
                  <span className="text-[10px] uppercase font-extrabold tracking-widest text-brand-cyan">
                    Lab Tests ({chartLabs.length})
                  </span>
                </div>
                {chartLabs.length === 0 ? (
                  <p className="text-xs text-text-secondary/50 py-2">No lab tests on file.</p>
                ) : (
                  <div className="space-y-2">
                    {chartLabs.map((l) => (
                      <div key={l.test_id} className="p-3 bg-[#112255]/30 border border-white/5 rounded-xl text-xs space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="text-white font-bold">{l.test_name}</span>
                          <span className="text-text-secondary font-mono text-[9px]">{l.test_date}</span>
                        </div>
                        {l.result && <p className="text-text-secondary text-[10px]"><span className="text-brand-cyan font-bold">Result:</span> {l.result}</p>}
                        {l.notes && <p className="text-text-secondary/60 text-[9px] italic">{l.notes}</p>}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="flex items-center space-x-3 pt-4 border-t border-white/5">
                <Button variant="outline" onClick={() => setChartOpen(false)} className="flex-1 py-2 text-xs">
                  Close Chart
                </Button>
                <Button
                  onClick={() => {
                    setChartOpen(false);
                    toast.success(`Starting consultation for ${chartPatient.name}`);
                  }}
