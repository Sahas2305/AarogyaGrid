...
import { Search, Bell, LogOut, ChevronDown, User, Shield, Stethoscope, Users, Building2, MapPin, Star } from 'lucide-react';
...
export const Topbar = ({ onBellClick, unreadCount = 3 }) => {
  ...
  return (
    <header className="h-16 bg-[#0d2044] border-b border-white/8 flex items-center justify-between px-6 z-20 relative select-none">
      ...
      {/* Right: Notification, Facility Switcher and Profiles */}
      <div className="flex items-center space-x-3">

        {/* Active Hospital Facility Pill */}
        {selectedHospital && (
          <button
            onClick={() => setHospitalModalOpen(true)}
            className="flex items-center space-x-2 px-3 py-1.5 bg-[#112255]/60 hover:bg-[#112255] border border-brand-cyan/30 hover:border-brand-cyan/60 rounded-xl transition-all text-left group"
            title="Click to switch hospital facility"
          >
            <div className="p-1 bg-brand-cyan/20 rounded-lg text-brand-cyan shrink-0">
              <Building2 className="w-3.5 h-3.5" />
            </div>
            <div className="hidden sm:block truncate max-w-[160px] md:max-w-[200px]">
              <span className="text-[9px] uppercase font-bold tracking-wider text-brand-cyan block leading-tight">
            ...
            <ChevronDown className="w-3.5 h-3.5 text-text-secondary group-hover:text-white transition-colors shrink-0" />
          </button>
        )}
